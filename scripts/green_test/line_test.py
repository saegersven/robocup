...
for x in range(image.shape[1]):
	for y in range(image.shape[0]):
		s = float(image[y][x][0]) + float(image[y][x][2])
		if(s == 0.0):
			continue
		ratio = float(image[y][x][1]) / s

		if(ratio > 0.85 and image[y][x][1] > 40):
			green[y][x] = 255
...
contours, hierarchy = cv2.findContours(green, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	...
	# if(num_contours != 1):
	# 	print(f"'{filename.__str__()}' wrong ({num_contours})")
	# 	cv2.imshow("Green", green)
...
#print(f"{err} wrong in {min(len(data), max_count)} images ({err / min(len(data), max_count) * 100.0}%)")
...
for i in range(len(contours)):
	...
	print(f"Initial Angle:\t{rad_to_deg(initial_angle)}")
	black_points = []
	for x in range(30):
		angle = deg_to_rad(360.0 / 30.0 * x)
		if(angle > deg_to_rad(180.0)):
			angle = angle - deg_to_rad(360.0)
		cx = r[0][0] + math.sin(angle) * (r[1][0] * 0.75)
		cy = r[0][1] - math.cos(angle) * (r[1][1] * 0.75)
		c = (0, 255, 0)
	...
	if(len(black_points) > 5):
		...
		print(f"Mean angle:\t{rad_to_deg(mean_angle)} ({len(black_points)})")
...
cv2.imwrite("image.png", image_debug)
